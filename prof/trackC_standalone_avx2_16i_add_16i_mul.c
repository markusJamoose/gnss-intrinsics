/*!
 *  \file trackC_standalone_avx2_16i_add_16i_mul.c
 ...
 *  \details    Profiles code when using:
 ...
 4. Multiplication of baseband signal with a local replica of ranging code using
 AVX2 SIMD intrinsics with si16 types
 5. Accumulation to generate the correlation value with saturation of AVX2 SIMD
 si16 types.
...
$ gcc -I ../src/ trackC_standalone_avx2_16i_add_16i_mul.c -g
 -mavx2 -lm -o avx2_si16_add_si16_mul -O3
 *  \endcode
 */
...
int main() {
  ...
  for (loopcount = 0; loopcount < codePeriods; loopcount++) {
    ...
    // Create vector to store iBasebandSignal times eCode
    short iBb_x_E[blksize];

    // Multiply eCode by iBasebandSignal and store in e_X_iBb
    avx2_mul_short(iBb_x_E, eCode_vec, mixedcarrSin_vec, blksize);

    // Accumulate elements of e_X_iBb and store in I_E
    I_E = avx_accumulate_short(iBb_x_E, blksize);

    // Repeat for other 5 values:

    // I_L
    short iBb_x_L[blksize];
    avx2_mul_short(iBb_x_L, lCode_vec, mixedcarrSin_vec, blksize);
    I_L = avx_accumulate_short(iBb_x_L, blksize);

    // I_P
    short iBb_x_P[blksize];
    avx2_mul_short(iBb_x_P, pCode_vec, mixedcarrSin_vec, blksize);
    I_P = avx_accumulate_short(iBb_x_P, blksize);

    // Q_E
    short qBb_x_E[blksize];
    avx2_mul_short(qBb_x_E, eCode_vec, mixedcarrCos_vec, blksize);
    Q_E = avx_accumulate_short(qBb_x_E, blksize);

    // Q_L
    short qBb_x_L[blksize];
    avx2_mul_short(qBb_x_L, lCode_vec, mixedcarrCos_vec, blksize);
    Q_L = avx_accumulate_short(qBb_x_L, blksize);

    // Q_P
    short qBb_x_P[blksize];
    avx2_mul_short(qBb_x_P, pCode_vec, mixedcarrCos_vec, blksize);
    Q_P = avx_accumulate_short(qBb_x_P, blksize);
  ...
  fclose(fpdata);
  printf(
      "  [Logging data into the 'plot/data_avx2_16i_add_16i_mul' directory]\n");

  // Clearing unused variables for logging operations
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/codeNco_output.bin",
                  codeNco_output);
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/codeError_output.bin",
                  codeError_output);
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/carrNco_output.bin",
                  carrNco_output);
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/carrError_output.bin",
                  carrError_output);
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/absoluteSample_output.bin",
                  absoluteSample_output);
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/carrFreq_output.bin",
                  carrFreq_output);
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/codeFreq_output.bin",
                  codeFreq_output);
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/I_E_output.bin",
                  I_E_output);
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/I_P_output.bin",
                  I_P_output);
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/I_L_output.bin",
                  I_L_output);
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/Q_E_output.bin",
                  Q_E_output);
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/Q_P_output.bin",
                  Q_P_output);
  write_file_fl64("../plot/data_avx2_16i_add_16i_mul/Q_L_output.bin",
                  Q_L_output);
