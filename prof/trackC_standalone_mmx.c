 ...
 * runs without MATLAB.  This version also uses MMX intrinsic functions
 *
 * Author: Jake Johnson
...
int main(){
  ...
  getcaCodeFromFile("data/caCode.bin", caCode);
	...
	FILE *fp = fopen("plot/data_avx2_16i_add_16i_mul_single_mulacc/I_E_output.bin","wb");
	fwrite(I_E_output, sizeof *I_E_output, 50000, fp);

	// Write I_P_output to bin file
	fp = fopen("plot/data_avx2_16i_add_16i_mul_single_mulacc/I_P_output.bin","wb");
	fwrite(I_P_output, sizeof *I_P_output, 50000, fp);

	// Write I_L_output to bin file
	fp = fopen("plot/data_avx2_16i_add_16i_mul_single_mulacc/I_L_output.bin","wb");
	fwrite(I_L_output, sizeof *I_L_output, 50000, fp);

	// Write Q_E_output to bin file
	fp = fopen("plot/data_avx2_16i_add_16i_mul_single_mulacc/Q_E_output.bin","wb");
	fwrite(Q_E_output, sizeof *Q_E_output, 50000, fp);

	// Write Q_P_output to bin file
	fp = fopen("plot/data_avx2_16i_add_16i_mul_single_mulacc/Q_P_output.bin","wb");
	fwrite(Q_P_output, sizeof *Q_P_output, 50000, fp);

	// Write Q_L_output to bin file
	fp = fopen("plot/data_avx2_16i_add_16i_mul_single_mulacc/Q_L_output.bin","wb");
...
//compile: gcc -g -mavx -mmx -march=haswell trackC_standalone_mmx.c -lm
