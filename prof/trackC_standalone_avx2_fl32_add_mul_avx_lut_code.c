...
int main() {
  ...
  FILE *fp =
      fopen("../plot/data_avx2_fl32_add_mul_avx_lut_code/I_E_output.bin", "wb");
  fwrite(I_E_output, sizeof *I_E_output, 50000, fp);

  // Write I_P_output to bin file
  fp =
      fopen("../plot/data_avx2_fl32_add_mul_avx_lut_code/I_P_output.bin", "wb");
  fwrite(I_P_output, sizeof *I_P_output, 50000, fp);

  // Write I_L_output to bin file
  fp =
      fopen("../plot/data_avx2_fl32_add_mul_avx_lut_code/I_L_output.bin", "wb");
  fwrite(I_L_output, sizeof *I_L_output, 50000, fp);

  // Write Q_E_output to bin file
  fp =
      fopen("../plot/data_avx2_fl32_add_mul_avx_lut_code/Q_E_output.bin", "wb");
  fwrite(Q_E_output, sizeof *Q_E_output, 50000, fp);

  // Write Q_P_output to bin file
  fp =
      fopen("../plot/data_avx2_fl32_add_mul_avx_lut_code/Q_P_output.bin", "wb");
  fwrite(Q_P_output, sizeof *Q_P_output, 50000, fp);

  // Write Q_L_output to bin file
  fp =
      fopen("../plot/data_avx2_fl32_add_mul_avx_lut_code/Q_L_output.bin", "wb");
