/*!
 *  \file trackC_standalone_avx2_32i_add_16i_mul.c
...
$ gcc -I ../src/ trackC_standalone_avx2_32i_add_16i_mul.c -g
 -mavx2 -lm -o avx2_si32_add_si16_mul -O3
 *  \endcode
 */
...
int main() {
  ...
  for (loopcount = 0; loopcount < codePeriods; loopcount++) {
    ...
    // Create vector to store qBasebandSignal times eCode (int (instead of
    // short) because performing 32bit adding in accumulator.)
    int qBb_x_E[blksize];

    // Multiply eCode by qBasebandSignal and store in e_X_iBb
    avx2_mul_short_store_int(qBb_x_E, eCode_vec, mixedcarrSin_vec, blksize);

    // Accumulate elements of e_X_iBb and store in I_E
    Q_E = avx_accumulate_int(qBb_x_E, blksize);

    // Repeat for other 5 values:

    // Q_L
    int qBb_x_L[blksize];
    avx2_mul_short_store_int(qBb_x_L, lCode_vec, mixedcarrSin_vec, blksize);
    Q_L = avx_accumulate_int(qBb_x_L, blksize);

    // Q_P
    int qBb_x_P[blksize];
    avx2_mul_short_store_int(qBb_x_P, pCode_vec, mixedcarrSin_vec, blksize);
    Q_P = avx_accumulate_int(qBb_x_P, blksize);

    // I_E
    int iBb_x_E[blksize];
    avx2_mul_short_store_int(iBb_x_E, eCode_vec, mixedcarrCos_vec, blksize);
    I_E = avx_accumulate_int(iBb_x_E, blksize);

    // I_L
    int iBb_x_L[blksize];
    avx2_mul_short_store_int(iBb_x_L, lCode_vec, mixedcarrCos_vec, blksize);
    I_E = avx_accumulate_int(iBb_x_L, blksize);

    // I_P
    int iBb_x_P[blksize];
    avx2_mul_short_store_int(iBb_x_P, pCode_vec, mixedcarrCos_vec, blksize);
    I_P = avx_accumulate_int(iBb_x_P, blksize);
  ...
  fclose(fpdata);
  printf(
      "  [Logging data into the 'plot/data_avx2_32i_add_16i_mul' directory]\n");

  // Clearing unused variables for logging operations
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/codeNco_output.bin",
                  codeNco_output);
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/codeError_output.bin",
                  codeError_output);
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/carrNco_output.bin",
                  carrNco_output);
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/carrError_output.bin",
                  carrError_output);
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/absoluteSample_output.bin",
                  absoluteSample_output);
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/carrFreq_output.bin",
                  carrFreq_output);
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/codeFreq_output.bin",
                  codeFreq_output);
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/I_E_output.bin",
                  I_E_output);
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/I_P_output.bin",
                  I_P_output);
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/I_L_output.bin",
                  I_L_output);
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/Q_E_output.bin",
                  Q_E_output);
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/Q_P_output.bin",
                  Q_P_output);
  write_file_fl64("../plot/data_avx2_32i_add_16i_mul/Q_L_output.bin",
                  Q_L_output);
