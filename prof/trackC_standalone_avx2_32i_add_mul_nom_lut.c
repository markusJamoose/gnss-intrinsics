...
int main(){
  ...
  getcaCodeFromFile("../data/caCode.bin", caCode);
	...
	const float delta_phi = (float) carrFreqBasis / samplingFreq * blksize; // phase increment
	...
	for (loopcount=0;loopcount<codePeriods;loopcount++)
	{
		...
		for(i=0;i<blksize;i++)
		{
			...
			eCode	=	(int32_t)(baseCode-earlyLateSpc)<(baseCode-earlyLateSpc)?(baseCode-earlyLateSpc+1):(baseCode-earlyLateSpc);
			lCode =	(int32_t)(baseCode+earlyLateSpc)<(baseCode+earlyLateSpc)?(baseCode+earlyLateSpc+1):(baseCode+earlyLateSpc);
		...
		avx2_nom_nco_si32(sin_nco_si32, sin_LUT_si32, blksize, remCarrPhase, carrFreq, samplingFreq);
		avx2_nom_nco_si32(cos_nco_si32, cos_LUT_si32, blksize, remCarrPhase, carrFreq, samplingFreq);

		// Mix to baseband
		avx2_si32_x2_mul_si32(mixedcarrSin_vec, sin_nco_si32, (int32_t *)rawSignal, blksize);
		avx2_si32_x2_mul_si32(mixedcarrCos_vec, cos_nco_si32, (int32_t *)rawSignal, blksize);
