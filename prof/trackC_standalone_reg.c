  ...
  getcaCodeFromFile("data/caCode.bin", caCode);
	...
	*/
	...
	for (loopcount=0;loopcount<codePeriods;loopcount++)
	{
		...
		//blksize is supposed to be ceiling'd but for some reason compiler won't find <math.h> header... >:(
	...
	for(i=0;i<blksize;i++)
			{
				...
				{
					mixedcarrSin	=	(*(rawSignalI+i)*carrSin+*(rawSignalQ+i)*carrCos);
					mixedcarrCos	=	(*(rawSignalI+i)*carrCos-*(rawSignalQ+i)*carrSin);
				}

				baseCode	=	(i*codePhaseStep+remCodePhase);

				pCode	=	(int)(baseCode)<baseCode?(baseCode+1):baseCode;
				eCode	=	(int)(baseCode-earlyLateSpc)<(baseCode-earlyLateSpc)?(baseCode-earlyLateSpc+1):(baseCode-earlyLateSpc);
				lCode	=	(int)(baseCode+earlyLateSpc)<(baseCode+earlyLateSpc)?(baseCode+earlyLateSpc+1):(baseCode+earlyLateSpc);
    ...
    // Store values in output arrays         vvv Corresponding variable in Matlab (trackResults)
		...
		*(lhs13+loopcount)	=	Q_L;
		...
		if ((loopcount%100)==0)
		{
				...
				sprintf(trackingStatusUpdated,"%s\n Completed: %d of %d\nC/No: %4.1f (dB-Hz)",trackingStatus,loopcount,codePeriods,CNo);
				pr[0] = mxCreateDoubleScalar((double)loopcount/codePeriods);
				pr[1] = hwb;
				pr[2] = mxCreateString(trackingStatusUpdated);
				mexCallMATLAB(0,NULL,3, pr, "waitbar");
